 ...
 * 时间索引编码
 * 
 * 解决问题： 在数据仓库（hadoop->hive etc.）表和表做业务join，连接字段就比较复杂,如果需要对相似时间delta T
 * 时间范围内做时间方向上统一，不好做，基于Mapreduce是可以的，对reduce line by line去过时间片就可以搞定，但是 对于hive等数据仓库，基于sql，需要join字段，但是
 * 空间 时间等，难以做相似位置，或者相似时间的统一和归一化。
 * 
...
public class TimeNormalization {
   ...
   * 
   * 
   * 取后N位不一致 3s -> 后2bit 2^2*0.734 --> 15位相同 12s -> 后4bit 2^4*0.734 --> 14位相同 50s -> 后6bit 2^6*0.734
   * --> 13位相同 3min -> 后8bit 2^8*0.734 --> 12位相同 12min -> 后10bit 2^10*0.734 --> 11位相同 50min ->
   * 后12bit 2^12*0.734 --> 10位相同 200min -> 后14bit 2^14*0.734 --> 9位相同 13.3h -> 后16bit 2^16*0.734 -->
   * 8位相同
   * 
  ...
  public Long deNormalize(String timeHash) {
    if (timeHash == null || timeHash.length() != MAX_PRECISION_16) {
      throw new IllegalArgumentException(
          "deNormalize use timehash string illegal,length must be 16");
    }
  ...
  /**
   * 3155731200000 -> 2070-01-01 00:00:00 timestamp 0 -> 1970-01-01 00:00:00 timestamp 把这个数字做2^32
   * 做二分，做二进制编码，时间范围(0~3155731200000)
   * 
   * 3155731200000/2^32 =~ 0.73s基本上满足业务使用
   * 
   * [min,mid) 二进制编码取 0 [mid,max] 二进制编码取 1
   * 
   */
